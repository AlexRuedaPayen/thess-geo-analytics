...
class CdseSceneCatalogService:
    """
    Service to search Sentinel scenes via CDSE STAC catalogue and return a tabular scene catalog.

    Owns:
      - reading AOI geometry from GeoJSON
      - querying STAC catalogue
      - returning a DataFrame of scenes (id/datetime/cloud/etc.)
    ...
    def search_scenes(
        ...
        query = {"eo:cloud_cover": {"lt": p.cloud_cover_max}} if p.cloud_cover_max is not None else None
    ...
    def smoke_test() -> None:
        ...
        aoi = Path("aoi/EL522_Thessaloniki.geojson") ####here it is specific to Thessaloniki
        if not aoi.exists():
